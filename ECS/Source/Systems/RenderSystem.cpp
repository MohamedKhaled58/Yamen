...
RenderSystem::RenderSystem(Graphics::GraphicsDevice &device,
    ...
    : m_Device(device), m_Renderer3D(renderer3D), m_Renderer2D(renderer2D),
      m_ShadowsEnabled(true) {
  YAMEN_CORE_INFO("RenderSystem created");
}

RenderSystem::~RenderSystem() { YAMEN_CORE_INFO("RenderSystem destroyed"); }
...
void RenderSystem::OnShutdown(Scene *scene) { m_ShadowMap.reset(); }
...
void RenderSystem::RenderShadowPass(Scene *scene, Graphics::Camera3D *camera) {
  ...
  for (auto entity : meshView) {
    ...
    m_Renderer3D->DrawMeshWithSubMeshes(mesh.Mesh.get(),
...
void RenderSystem::RenderOpaquePass(Scene *scene, Graphics::Camera3D *camera) {
  ...
  for (auto entity : meshView) {
    ...
    renderQueue.push_back({entity, mesh.Mesh.get(), mesh.Material.get(),
                           transform.GetTransform()});
  }
...
void RenderSystem::RenderTransparentPass(Scene *scene,
  ...
  for (auto entity : meshView) {
    ...
    if (!mesh.Visible || !mesh.Mesh || !mesh.Material)
      continue;
    if (!mesh.Material->GetBlendState())
      continue;

    float distSq = Math::LengthSq(transform.Translation - camPos);
  ...
  for (auto &item : transparentList) {
    ...
    m_Renderer3D->DrawMesh(mesh.Mesh.get(), transform.GetTransform(),
                           mesh.Material.get());
  }
}
...
void RenderSystem::Render2DPass(Scene *scene) {
  ...
  for (auto entity : sorted) {
    ...
    vec2 pos(transform.Translation.x, transform.Translation.y);
    vec2 scale(transform.Scale.x, transform.Scale.y);

    if (sprite.Texture) {
      m_Renderer2D->DrawSprite(sprite.Texture.get(), pos, scale,
                               transform.Rotation.z, sprite.Color);
    } else {
      m_Renderer2D->DrawQuad(pos, scale, sprite.Color, transform.Rotation.z);
    }
  }
