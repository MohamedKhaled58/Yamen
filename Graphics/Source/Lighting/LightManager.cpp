...
namespace Yamen::Graphics {
    ...
    std::vector<Light*> LightManager::GetLightsForObject(const glm::vec3& position, float radius) const {
        ...
        for (auto* light : m_Lights) {
            ...
            float distance = glm::distance(light->position, position);
    ...
    std::vector<Light*> LightManager::GetTopLights(const glm::vec3& position, size_t maxLights) const {
    ...
    float LightManager::CalculateLightImportance(const Light* light, const glm::vec3& position) const {
