...
namespace Yamen::Graphics {
    ...
    bool Texture2D::Create(uint32_t width, uint32_t height, TextureFormat format, const void* data, bool generateMips) {
        m_Width = width;
        m_Height = height;
        m_Format = format;
        ...
        if (FAILED(hr)) {
            YAMEN_CORE_ERROR("Failed to create texture: 0x{:08X}", static_cast<uint32_t>(hr));
            return false;
        }
        ...
        hr = m_Device.GetDevice()->CreateShaderResourceView(
            m_Texture.Get(),
            &srvDesc,
            &m_SRV
        );
        ...
        if (FAILED(hr)) {
            YAMEN_CORE_ERROR("Failed to create shader resource view: 0x{:08X}", static_cast<uint32_t>(hr));
            return false;
        }
        ...
        YAMEN_CORE_TRACE("Created texture ({}x{}, {} mips)", width, height, m_MipLevels);
        return true;
    }
