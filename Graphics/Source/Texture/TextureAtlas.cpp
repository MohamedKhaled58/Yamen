...
bool TextureAtlas::Load(GraphicsDevice &device, const std::string &imagePath,
  ...
  while (file >> name >> x >> y >> width >> height) {
    AtlasRegion region;
    region.name = name;
    region.offset = vec2(x, y);
    region.size = vec2(width, height);
    region.uvMin = vec2(x / m_Width, y / m_Height);
    region.uvMax = vec2((x + width) / m_Width, (y + height) / m_Height);

    m_Regions[name] = region;
  }

  YAMEN_CORE_INFO("Loaded texture atlas: {} regions from {}", m_Regions.size(),
                  imagePath);
  return true;
}

bool TextureAtlas::Create(GraphicsDevice &device,
  ...
  YAMEN_CORE_WARN("TextureAtlas::Create not fully implemented. Use Load() with "
                  "pre-packed atlas.");
  (void)device;
  (void)imagePaths;
  (void)maxSize;
