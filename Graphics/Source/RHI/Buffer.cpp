...
namespace Yamen::Graphics {
    ...
    bool Buffer::Create(const void* data, uint32_t size, uint32_t stride, BufferUsage usage) {
        m_Size = size;
        m_Stride = stride;
        m_Usage = usage;
        ...
        // Create buffer
        HRESULT hr = m_Device.GetDevice()->CreateBuffer(&bufferDesc, pInitData, &m_Buffer);
        if (FAILED(hr)) {
            YAMEN_CORE_ERROR("Failed to create buffer: 0x{:08X}", static_cast<uint32_t>(hr));
            return false;
        }
        ...
        switch (m_Type) {
            ...
            case BufferType::Constant: typeStr = "Constant"; break;
        }

        YAMEN_CORE_TRACE("Created {} buffer ({} bytes, stride: {})", typeStr, size, stride);
        return true;
    }

    void Buffer::Update(const void* data, uint32_t size) {
        if (!m_Buffer || m_Usage != BufferUsage::Dynamic) {
