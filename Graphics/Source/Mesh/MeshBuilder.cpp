...
void MeshBuilder::CreateCube(std::vector<Vertex3D> &vertices,
                             std::vector<uint32_t> &indices, float size) {
  float half = size * 0.5f;

  // 24 vertices (4 per face for proper normals)
  vertices = {// Front face (+Z)
...
void MeshBuilder::CreateSphere(std::vector<Vertex3D> &vertices,
                               std::vector<uint32_t> &indices, float radius,
  ...
  for (uint32_t ring = 0; ring <= rings; ++ring) {
    ...
    for (uint32_t segment = 0; segment <= segments; ++segment) {
      float theta = 2.0f * Math::PI * static_cast<float>(segment) /
                    static_cast<float>(segments);
      float x = ringRadius * cos(theta);
      float z = ringRadius * sin(theta);

      vec3 position(x, y, z);
      vec3 normal = Math::Normalize(position);
      vec2 texCoord(static_cast<float>(segment) / static_cast<float>(segments),
...
void MeshBuilder::CreatePlane(std::vector<Vertex3D> &vertices,
                              std::vector<uint32_t> &indices, float width,
                              float height, uint32_t subdivisionsX,
  ...
  for (uint32_t y = 0; y <= subdivisionsY; ++y) {
    for (uint32_t x = 0; x <= subdivisionsX; ++x) {
      ...
      vec3 position(-halfWidth + u * width, 0.0f, -halfHeight + v * height);
...
void MeshBuilder::CalculateTangents(std::vector<Vertex3D> &vertices,
