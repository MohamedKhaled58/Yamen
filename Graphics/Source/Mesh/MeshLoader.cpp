...
std::unique_ptr<Mesh> MeshLoader::LoadOBJ(GraphicsDevice &device,
  ...
  while (std::getline(file, line)) {
    ...
    if (prefix == "v") {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        // TODO Check ??
        vertex.tangent = vec3(1.0f); // Default white
  ...
  YAMEN_CORE_INFO("Loaded OBJ mesh: {0} (Vertices: {1}, Indices: {2})",
