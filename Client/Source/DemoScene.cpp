...
DemoScene::DemoScene(Graphics::GraphicsDevice &device)
    : m_Device(device), m_Rotation(0.0f), m_ShowWireframe(false),
      m_Show2D(true), m_Show3D(true), m_LightDirection(0.0f, -1.0f, 0.3f),
      m_LightColor(1.0f, 0.95f, 0.8f), m_UseMaterials(false) {}
...
bool DemoScene::Initialize() {
  ...
  m_Camera3D =
      std::make_unique<Graphics::Camera3D>(60.0f, 16.0f / 9.0f, 0.1f, 1000.0f);
  m_Camera3D->SetPosition(0.0f, 5.0f, -15.0f);
  // Yaw 90 degrees to look at +Z (where objects are), Pitch -15 to look
  // slightly down
  m_Camera3D->SetRotation(
      vec3(Math::Radians(-15.0f), Math::Radians(90.0f), 0.0f));
  ...
  // Setup lights
  m_SunLight = Graphics::Light::CreateDirectional(
      vec3(0.3f, -1.0f, 0.2f), vec3(1.0f, 0.95f, 0.8f), 0.8f);
  m_PointLight1 =
      Graphics::Light::CreatePoint(vec3(-5, 3, 5), vec3(1, 0, 0), 2.0f, 10.0f);
  m_PointLight2 =
      Graphics::Light::CreatePoint(vec3(5, 3, 5), vec3(0, 0, 1), 2.0f, 10.0f);
...
void DemoScene::CreateTestTextures() {
  ...
  m_TestTexture = Graphics::TextureLoader::CreateSolidColor(m_Device, 64, 64,
                                                            255, 255, 255, 255);
...
void DemoScene::CreateTestMaterials() {
  ...
  m_RedMaterial->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                            m_TestTexture.get());
  m_RedMaterial->SetVector(Graphics::Material::ALBEDO_COLOR, vec4(1, 0, 0, 1));

  // Green material
  m_GreenMaterial = std::make_unique<Graphics::Material>();
  m_GreenMaterial->SetShader(shader);
  m_GreenMaterial->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                              m_TestTexture.get());
  m_GreenMaterial->SetVector(Graphics::Material::ALBEDO_COLOR,
                             vec4(0, 1, 0, 1));

  // Blue material
  m_BlueMaterial = std::make_unique<Graphics::Material>();
  m_BlueMaterial->SetShader(shader);
  m_BlueMaterial->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                             m_TestTexture.get());
  m_BlueMaterial->SetVector(Graphics::Material::ALBEDO_COLOR, vec4(0, 0, 1, 1));
...
void DemoScene::Render() {
  ...
  if (m_Show3D) {
    ...
    m_SunLight.direction = Math::Normalize(m_LightDirection);
    ...
    // Transforms
    mat4 planeTransform = Math::Translate(mat4(1.0f), vec3(0.0f, -2.0f, 0.0f));
    mat4 cubeTransform = Math::Translate(mat4(1.0f), vec3(-3.0f, 0.0f, 0.0f));
    cubeTransform =
        Math::Rotate(cubeTransform, m_Rotation, vec3(0.0f, 1.0f, 0.0f));
    mat4 sphereTransform = Math::Translate(mat4(1.0f), vec3(3.0f, 0.0f, 0.0f));
    sphereTransform = Math::Rotate(sphereTransform, m_Rotation * 0.7f,
                                   vec3(1.0f, 0.0f, 1.0f));

    if (m_UseMaterials) {
      // Render with materials (different colors)
      m_Renderer3D->DrawMesh(m_PlaneMesh.get(), planeTransform,
                             m_GreenMaterial.get());
      m_Renderer3D->DrawMesh(m_CubeMesh.get(), cubeTransform,
                             m_RedMaterial.get());
      m_Renderer3D->DrawMesh(m_SphereMesh.get(), sphereTransform,
                             m_BlueMaterial.get());
    } else {
      // Original rendering
      m_Renderer3D->DrawMesh(m_PlaneMesh.get(), planeTransform, nullptr,
                             vec4(0.3f, 0.5f, 0.3f, 1.0f));
      m_Renderer3D->DrawMesh(m_CubeMesh.get(), cubeTransform, nullptr,
                             vec4(1.0f, 0.3f, 0.3f, 1.0f));
      m_Renderer3D->DrawMesh(m_SphereMesh.get(), sphereTransform, nullptr,
  ...
  if (m_Show2D) {
    ...
    m_Renderer2D->DrawQuad(vec2(100.0f, 100.0f), vec2(100.0f, 100.0f),
                           vec4(1.0f, 0.0f, 0.0f, 1.0f));

    m_Renderer2D->DrawQuad(vec2(220.0f, 100.0f), vec2(100.0f, 100.0f),
                           vec4(0.0f, 1.0f, 0.0f, 1.0f));

    m_Renderer2D->DrawQuad(vec2(340.0f, 100.0f), vec2(100.0f, 100.0f),
