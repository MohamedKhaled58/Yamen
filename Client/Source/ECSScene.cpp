...
ECSScene::ECSScene(Graphics::GraphicsDevice &device) : m_Device(device) {}
...
bool ECSScene::Initialize() {
  ...
  m_Material->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                         m_WhiteTexture.get());
  m_Material->SetVector(Graphics::Material::ALBEDO_COLOR,
  ...
  cameraTransform.Translation = vec3(0.0f, 10.0f, -20.0f);
  ...
  lightComp.LightData = Graphics::Light::CreateDirectional(
      Math::Normalize(vec3(-0.3f, -1.0f, -0.3f)), vec3(1.0f, 1.0f, 1.0f), 3.0f);
  ...
  vec3 direction = Math::Normalize(vec3(-0.3f, -1.0f, -0.3f));
  // Use LookAtLH to get view matrix, then inverse to get world matrix, then
  // extract rotation
  lightTrans.Rotation = Math::ToQuat(Math::Inverse(
      Math::LookAtLH(vec3(0.0f), direction, vec3(0.0f, 1.0f, 0.0f))));
...
void ECSScene::CreateCoordinateGizmos() {
  ...
  matAxisX->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                       m_WhiteTexture.get());
  matAxisX->SetVector(Graphics::Material::ALBEDO_COLOR,
                      vec4(1.0f, 0.0f, 0.0f, 1.0f));
  meshAxisX.Material = matAxisX;
  auto &transX = axisX.GetComponent<ECS::TransformComponent>();
  transX.Translation = vec3(5.0f, 0.0f, 0.0f);
  transX.Scale = vec3(10.0f, 0.2f, 0.2f);
  ...
  matAxisY->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                       m_WhiteTexture.get());
  matAxisY->SetVector(Graphics::Material::ALBEDO_COLOR,
                      vec4(0.0f, 1.0f, 0.0f, 1.0f));
  meshAxisY.Material = matAxisY;
  auto &transY = axisY.GetComponent<ECS::TransformComponent>();
  transY.Translation = vec3(0.0f, 5.0f, 0.0f);
  transY.Scale = vec3(0.2f, 10.0f, 0.2f);
  ...
  matAxisZ->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                       m_WhiteTexture.get());
  matAxisZ->SetVector(Graphics::Material::ALBEDO_COLOR,
                      vec4(0.0f, 0.0f, 1.0f, 1.0f));
  meshAxisZ.Material = matAxisZ;
  auto &transZ = axisZ.GetComponent<ECS::TransformComponent>();
  transZ.Translation = vec3(0.0f, 0.0f, 5.0f);
  transZ.Scale = vec3(0.2f, 0.2f, 10.0f);
}
...
void ECSScene::CreatePhysicsDemo() {
  ...
  groundMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                        m_WhiteTexture.get());
  groundMat->SetVector(Graphics::Material::ALBEDO_COLOR,
                       vec4(0.3f, 0.3f, 0.3f, 1.0f));
  groundMesh.Material = groundMat;

  auto &groundTrans = ground.GetComponent<ECS::TransformComponent>();
  groundTrans.Translation = vec3(0.0f, -0.5f, 0.0f);
  groundTrans.Scale = vec3(50.0f, 1.0f, 50.0f);

  auto &groundRb = ground.AddComponent<ECS::RigidBodyComponent>();
  groundRb.Type = ECS::BodyType::Static;

  ECS::BoxCollider groundBox;
  groundBox.HalfExtents = vec3(25.0f, 0.5f, 25.0f);
  ...
  testMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                      m_WhiteTexture.get());
  testMat->SetVector(Graphics::Material::ALBEDO_COLOR,
                     vec4(1.0f, 0.5f, 0.0f, 1.0f));
  testMesh.Material = testMat;

  auto &testTrans = testCube.GetComponent<ECS::TransformComponent>();
  testTrans.Translation = vec3(0.0f, 1.0f, 0.0f);
  testTrans.Scale = vec3(2.0f);
  ...
  physBoxMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                         m_WhiteTexture.get());
  physBoxMat->SetVector(Graphics::Material::ALBEDO_COLOR,
                        vec4(0.0f, 1.0f, 1.0f, 1.0f));
  physBoxMesh.Material = physBoxMat;

  auto &physBoxTrans = physBox.GetComponent<ECS::TransformComponent>();
  physBoxTrans.Translation = vec3(-5.0f, 10.0f, 0.0f);
  physBoxTrans.Scale = vec3(1.5f);
  ...
  physBoxCol.HalfExtents = vec3(0.75f);
  ...
  test2Mat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                       m_WhiteTexture.get());
  test2Mat->SetVector(Graphics::Material::ALBEDO_COLOR,
                      vec4(1.0f, 0.0f, 1.0f, 1.0f));
  test2Mesh.Material = test2Mat;

  auto &test2Trans = testCube2.GetComponent<ECS::TransformComponent>();
  test2Trans.Translation = vec3(5.0f, 1.0f, 0.0f);
  test2Trans.Scale = vec3(2.0f);
}
...
void ECSScene::RenderImGui() {
  ...
  if (gizmoSystem) {
    ...
    int currentMode = (gizmoSystem->GetOperation() == ImGuizmo::TRANSLATE) ? 0
                      : (gizmoSystem->GetOperation() == ImGuizmo::ROTATE)  ? 1
                                                                           : 2;
    ...
    if (m_SelectedEntity != entt::null) {
      auto &selectedTag =
          m_Scene->Registry().get<ECS::TagComponent>(m_SelectedEntity);
      ImGui::TextColored(ImVec4(1, 1, 0, 1), "Selected: %s",
                         selectedTag.Tag.c_str());
    } else {
  ...
  for (auto entity : view) {
    ...
    bool opened = ImGui::TreeNodeEx((void *)(uint64_t)entity, flags, "%s",
                                    tag.Tag.c_str());
    ...
    if (ImGui::BeginPopupContextItem()) {
      if (ImGui::MenuItem("Delete Entity")) {
        ...
        if (m_SelectedEntity == entity) {
          ...
          if (gizmoSystem)
            gizmoSystem->SetSelectedEntity(entt::null);
        }
      }
      ImGui::EndPopup();
    }
  ...
  if (m_SelectedEntity != entt::null) {
    ...
    if (registry.all_of<ECS::TransformComponent>(m_SelectedEntity)) {
      if (ImGui::CollapsingHeader("Transform",
        ...
        auto &transform =
            registry.get<ECS::TransformComponent>(m_SelectedEntity);

        ImGui::DragFloat3("Translation", &transform.Translation.x, 0.1f);

        vec3 rotation = Math::ToEulerAngles(transform.Rotation);
        ...
        if (ImGui::DragFloat3("Rotation", &rotation.x, 0.1f)) {
          transform.Rotation =
              quat(vec3(Math::Radians(rotation.x), Math::Radians(rotation.y),
    ...
    if (registry.all_of<ECS::RigidBodyComponent>(m_SelectedEntity)) {
      if (ImGui::CollapsingHeader("RigidBody",
        ...
        ImGui::Text("Velocity: %.2f, %.2f, %.2f", rb.Velocity.x, rb.Velocity.y,
                    rb.Velocity.z);
      }
    }
      ...
      if (ImGui::CollapsingHeader("Collider", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::Combo("Type", &currentType, colliderTypeStrings, 3)) {
          ...
          if (collider.Type == ECS::ColliderType::Box)
            collider.Shape = ECS::BoxCollider{};
          else if (collider.Type == ECS::ColliderType::Sphere)
            collider.Shape = ECS::SphereCollider{};
          else if (collider.Type == ECS::ColliderType::Capsule)
            collider.Shape = ECS::CapsuleCollider{};
        }
    ...
    if (registry.all_of<ECS::LightComponent>(m_SelectedEntity)) {
      if (ImGui::CollapsingHeader("Light", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::DragFloat("Intensity", &light.LightData.intensity, 0.1f, 0.0f,
                         100.0f);
  ...
  if (ImGui::CollapsingHeader("Physics", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (physicsSystem) {
      ImGui::DragFloat3("Gravity", &physicsSystem->Gravity.x, 0.1f, -50.0f,
                        50.0f);
  ...
  if (gizmoSystem && ImGui::CollapsingHeader("Gizmo Controls",
                                             ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::Combo("Operation", &currentOp, operations, 3)) {
      if (currentOp == 0)
        gizmoSystem->SetOperation(ImGuizmo::TRANSLATE);
      else if (currentOp == 1)
        gizmoSystem->SetOperation(ImGuizmo::ROTATE);
      else
        gizmoSystem->SetOperation(ImGuizmo::SCALE);
    }
    ...
    if (ImGui::Combo("Space", &currentSpace, spaces, 2)) {
      gizmoSystem->SetMode((currentSpace == 0) ? ImGuizmo::WORLD
                                               : ImGuizmo::LOCAL);
    }
