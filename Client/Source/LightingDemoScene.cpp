...
namespace Yamen {
    ...
    void LightingDemoScene::CreateEnvironment() {
        ...
        material->SetVector(Graphics::Material::ALBEDO_COLOR, glm::vec4(0.2f, 0.3f, 0.2f, 1.0f));
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            pillarMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE, m_WhiteTexture.get());
            pillarMat->SetVector(Graphics::Material::ALBEDO_COLOR, glm::vec4(0.7f, 0.7f, 0.7f, 1.0f));
            pillarMesh.Material = pillarMat;
            
            auto& pillarTransform = pillar.GetComponent<ECS::TransformComponent>();
            pillarTransform.Translation = glm::vec3(
                cos(angle) * radius,
                2.5f,
                sin(angle) * radius
            );
            pillarTransform.Scale = glm::vec3(1.0f, 5.0f, 1.0f);
        }
        ...
        sphereMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE, m_WhiteTexture.get());
        sphereMat->SetVector(Graphics::Material::ALBEDO_COLOR, glm::vec4(0.9f, 0.9f, 0.9f, 1.0f));
        sphereMesh.Material = sphereMat;
        
        auto& sphereTransform = centerSphere.GetComponent<ECS::TransformComponent>();
        sphereTransform.Translation = glm::vec3(0.0f, 2.0f, 0.0f);
    ...
    void LightingDemoScene::CreateDirectionalLight() {
        ...
        lightComp.LightData = Graphics::Light::CreateDirectional(
            ...
            glm::vec3(1.0f, 0.9f, 0.8f), // Warm sunlight
            1.5f
        );
    ...
    void LightingDemoScene::CreatePointLights() {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            auto& lightComp = light.AddComponent<ECS::LightComponent>();
            lightComp.LightData = Graphics::Light::CreatePoint(
                glm::vec3(0.0f),
                colors[i],
                3.0f,
                10.0f
            ...
            material->SetShader(m_Shader.get());
            material->SetTexture(Graphics::Material::DIFFUSE_TEXTURE, m_WhiteTexture.get());
            material->SetVector(Graphics::Material::ALBEDO_COLOR, glm::vec4(colors[i], 1.0f));
            mesh.Material = material;
    ...
    void LightingDemoScene::UpdateLights(float deltaTime) {
        ...
        if (m_DirectionalLight != entt::null) {
            auto& lightComp = m_Scene->Registry().get<ECS::LightComponent>(m_DirectionalLight);
            ...
            lightComp.LightData.color = glm::mix(
                glm::vec3(0.2f, 0.2f, 0.4f), // Night (blue)
                glm::vec3(1.0f, 0.9f, 0.7f), // Day (warm)
                dayFactor
            );
        ...
        for (size_t i = 0; i < m_PointLights.size(); ++i) {
            if (m_PointLights[i] == entt::null) continue;
            
            auto& transform = m_Scene->Registry().get<ECS::TransformComponent>(m_PointLights[i]);
            auto& lightComp = m_Scene->Registry().get<ECS::LightComponent>(m_PointLights[i]);
            
            float angle = m_TimeOfDay + i * glm::pi<float>() * 0.5f;
            float radius = 5.0f;
    ...
    void LightingDemoScene::RenderImGui() {
        ...
        ImGui::SliderFloat("Time of Day", &m_TimeOfDay, 0.0f, glm::two_pi<float>());
        
        ImGui::Separator();
        ImGui::Text("Light Count: %d", 1 + m_PointLights.size() + 1); // Dir + Points + Spot
        
        if (ImGui::CollapsingHeader("Directional Light")) {
            if (m_DirectionalLight != entt::null) {
                auto& lightComp = m_Scene->Registry().get<ECS::LightComponent>(m_DirectionalLight);
                ImGui::ColorEdit3("Color", &lightComp.LightData.color.x);
                ImGui::DragFloat("Intensity", &lightComp.LightData.intensity, 0.1f, 0.0f, 10.0f);
