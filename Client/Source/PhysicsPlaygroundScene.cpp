...
void PhysicsPlaygroundScene::CreateGround() {
  ...
  collider.HalfExtents = vec3(15.0f, 0.5f, 15.0f);
...
void PhysicsPlaygroundScene::CreatePyramid() {
  ...
  for (int level = 0; level < levels; ++level) {
    ...
    for (int i = 0; i < boxesInLevel; ++i) {
      ...
      vec3 color = vec3(1.0f - hue, hue, 0.5f);
      material->SetVector(Graphics::Material::ALBEDO_COLOR, vec4(color, 1.0f));
      mesh.Material = material;

      auto &transform = box.GetComponent<ECS::TransformComponent>();
      transform.Translation =
          vec3(startX + i * boxSize + level * boxSize * 0.5f,
               startY + level * boxSize, startZ);
      transform.Scale = vec3(0.9f);
      ...
      ECS::BoxCollider collider;
      collider.HalfExtents = vec3(0.45f);
...
void PhysicsPlaygroundScene::CreateDominoChain() {
  ...
  for (int i = 0; i < dominoCount; ++i) {
    ...
    transform.Translation = vec3(startX, 1.5f, startZ + i * spacing);
    transform.Scale = vec3(0.2f, 3.0f, 1.0f);
    ...
    ECS::BoxCollider collider;
    collider.HalfExtents = vec3(0.1f, 1.5f, 0.5f);
...
void PhysicsPlaygroundScene::CreateBouncyBalls() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    material->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                         m_WhiteTexture.get());
    material->SetVector(
        Graphics::Material::ALBEDO_COLOR,
        vec4(colorDist(gen), colorDist(gen), colorDist(gen), 1.0f));
    mesh.Material = material;

    auto &transform = ball.GetComponent<ECS::TransformComponent>();
    transform.Translation = vec3(posX(gen), 5.0f + i * 2.0f, posZ(gen));
...
void PhysicsPlaygroundScene::CreateHeavyBox() {
  ...
  auto &transform = ball.GetComponent<ECS::TransformComponent>();
  transform.Translation = vec3(-8.0f, 8.0f, 0.0f);
  transform.Scale = vec3(2.0f);

  auto &rb = ball.AddComponent<ECS::RigidBodyComponent>();
  rb.Type = ECS::BodyType::Dynamic;
  rb.Mass = 10.0f;                      // Heavy!
  rb.Velocity = vec3(5.0f, 0.0f, 0.0f); // Initial push
...
void PhysicsPlaygroundScene::SpawnRandomObject() {
  ...
  transform.Translation = vec3(posX(gen), 10.0f, posZ(gen));
  ...
  if (isSphere) {
    ...
    ECS::BoxCollider collider;
    collider.HalfExtents = vec3(0.5f);
    entity.AddComponent<ECS::ColliderComponent>(collider);
  }
}
...
void PhysicsPlaygroundScene::RenderImGui() {
  ...
  ImGui::Text("Entity Count: %d",
              m_Scene->Registry().view<ECS::TagComponent>().size());
