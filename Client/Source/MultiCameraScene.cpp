...
bool MultiCameraScene::Initialize() {
  ...
  playerMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                        m_WhiteTexture.get());
  playerMat->SetVector(Graphics::Material::ALBEDO_COLOR,
                       vec4(1.0f, 0.3f, 0.3f, 1.0f));
  playerMesh.Material = playerMat;

  auto &playerTransform = player.GetComponent<ECS::TransformComponent>();
  playerTransform.Translation = vec3(0.0f, 1.0f, 0.0f);
  playerTransform.Scale = vec3(1.5f);
...
void MultiCameraScene::CreateMainCamera() {
  ...
  transform.Translation = vec3(0.0f, 5.0f, -15.0f);
  transform.Rotation = quat(vec3(0.0f, Math::Radians(90.0f), 0.0f));
...
void MultiCameraScene::CreateTopDownCamera() {
  ...
  auto &transform = camera.GetComponent<ECS::TransformComponent>();
  transform.Translation = vec3(0.0f, 30.0f, 0.0f);
  transform.Rotation =
...
void MultiCameraScene::CreateFollowCamera() {
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    obsMat->SetTexture(Graphics::Material::DIFFUSE_TEXTURE,
                       m_WhiteTexture.get());
    obsMat->SetVector(Graphics::Material::ALBEDO_COLOR,
                      vec4(0.5f + i * 0.1f, 0.5f, 1.0f - i * 0.1f, 1.0f));
    obsMesh.Material = obsMat;

    auto &obsTransform = obstacle.GetComponent<ECS::TransformComponent>();
    obsTransform.Translation = vec3(-10.0f + i * 5.0f, 2.0f, 5.0f);
    obsTransform.Scale = vec3(1.0f, 4.0f, 1.0f);
  }
  ...
  lightComp.LightData = Graphics::Light::CreateDirectional(
      Math::Normalize(vec3(-0.5f, -1.0f, -0.3f)), vec3(1.0f, 1.0f, 1.0f), 2.0f);
...
void MultiCameraScene::Update(float deltaTime) {
  if (m_Scene) {
    if (m_FollowCamera != entt::null && m_PlayerCube != entt::null) {
      auto &followTransform =
          m_Scene->Registry().get<ECS::TransformComponent>(m_FollowCamera);
      auto &playerTransform =
          m_Scene->Registry().get<ECS::TransformComponent>(m_PlayerCube);

      vec3 offset = vec3(0.0f, 3.0f, -8.0f);
      followTransform.Translation = playerTransform.Translation + offset;

      vec3 direction = Math::Normalize(playerTransform.Translation -
                                       followTransform.Translation);
      float pitch = asin(-direction.y);
      float yaw = atan2(direction.x, direction.z);
      followTransform.Rotation = Math::AngleAxis(yaw, vec3(0.0f, 1.0f, 0.0f)) *
                                 Math::AngleAxis(pitch, vec3(1.0f, 0.0f, 0.0f));
    }
...
void MultiCameraScene::RenderImGui() {
  ...
  if (ImGui::RadioButton("Picture-in-Picture",
