...
namespace Yamen::World {
    ...
    // In CreateChild:
    static QuadTree* CreateChild(const Core::AABB& bounds, int capacity, int maxDepth, int depth) {
    ...
    bool QuadTree::Insert(entt::entity entity, const Core::AABB& bounds) {
        ...
        for (int i = 0; i < 4; ++i) {
            if (m_Children[i]->Insert(entity, bounds)) {
                ...
                // Actually, standard quadtree usually stores in the node that fully contains it
                // OR stores in all intersecting nodes (reference).
                // For simplicity here, we store in all intersecting nodes.
            }
        }
    ...
    void QuadTree::Query(const Core::AABB& range, std::vector<entt::entity>& found) const {
    ...
    void QuadTree::Query(const Core::Frustum& frustum, std::vector<entt::entity>& found) const {
        if (!frustum.IntersectsAABB(m_Bounds)) {
            return;
        }
    ...
    void QuadTree::Subdivide() {
        ...
        Core::AABB nwBounds(
            Core::vec3(min.x, min.y, center.z),
            Core::vec3(center.x, max.y, max.z)
        );

        // NE: center.x, center.z -> max.x, max.z
        Core::AABB neBounds(
            Core::vec3(center.x, min.y, center.z),
            Core::vec3(max.x, max.y, max.z)
        );

        // SW: min.x, min.z -> center.x, center.z
        Core::AABB swBounds(
            Core::vec3(min.x, min.y, min.z),
            Core::vec3(center.x, max.y, center.z)
        );

        // SE: center.x, min.z -> max.x, center.z
        Core::AABB seBounds(
            Core::vec3(center.x, min.y, min.z),
            Core::vec3(max.x, max.y, center.z)
        );

        m_Children[0].reset(CreateChild(nwBounds, m_Capacity, m_MaxDepth, m_Depth + 1));
        m_Children[1].reset(CreateChild(neBounds, m_Capacity, m_MaxDepth, m_Depth + 1));
        m_Children[2].reset(CreateChild(swBounds, m_Capacity, m_MaxDepth, m_Depth + 1));
        m_Children[3].reset(CreateChild(seBounds, m_Capacity, m_MaxDepth, m_Depth + 1));
