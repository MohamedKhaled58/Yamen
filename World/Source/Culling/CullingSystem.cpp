...
void CullingSystem::Update(const mat4 &viewProj) { m_Frustum.Update(viewProj); }

void CullingSystem::RegisterObject(uint32_t entityID, const vec3 &pos,
                                   float radius) {
  CullableObject obj;
  ...
  obj.BoxMin = pos - vec3(radius);
  obj.BoxMax = pos + vec3(radius);

  m_Objects.push_back(obj);
}

void CullingSystem::RegisterObject(uint32_t entityID, const vec3 &min,
                                   const vec3 &max) {
  CullableObject obj;
  obj.EntityID = entityID;
  obj.BoxMin = min;
  ...
  obj.Radius = Math::Length(max - min) * 0.5f;
  obj.IsVisible = true;
...
void CullingSystem::UnregisterObject(uint32_t entityID) {
  m_Objects.erase(std::remove_if(m_Objects.begin(), m_Objects.end(),
                                 [entityID](const CullableObject &obj) {
